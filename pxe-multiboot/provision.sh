...
configuration(){
  ...
  [ -e /var/lib/tftpboot/vesamenu.c32 ]        || cp /usr/lib/syslinux/vesamenu.c32 /var/lib/tftpboot/
  [ -e $VAGRANT/new_iso ]                      && install -o root -g root -m 755 $VAGRANT/new_iso /usr/local/bin/new_iso
...
cat <<EOF > /etc/exports
/srv/install                  *(ro,async,no_root_squash,no_subtree_check)
EOF
...
[ -e $SYSLINUX ] && cp $SYSLINUX /var/lib/tftpboot
...
cat <<EOF > /var/lib/tftpboot/pxelinux.cfg/pxe.conf
...
ping -c 1 -W 2 8.8.8.8 1>/dev/null 2>/dev/null || die "Your vagrant bridge interface should be connected not to the internet!"
